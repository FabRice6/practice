...
CAUTION: Works but importing numpy is apparently not allowed...
...
class Grid(object):
    ...
    def __init__(self, pacman, grid) -> None:
        ...
        self.visited = np.empty((self.rows, self.columns))
    
    def initializeVisitedMatrix(self):
        rows = len(self.grid)
        columns = len(self.grid[0])
        self.visited = np.full((rows, columns), False, dtype=bool)
